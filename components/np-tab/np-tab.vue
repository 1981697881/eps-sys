<template>
	<view class="me-tabs" :class="{'tabs-fixed': fixed}" :style="{height: tabHeightVal, background: fixed ? bgColor : ''}">
		<scroll-view v-if="tabs.length" :id="viewId" :scroll-left="scrollLeft" scroll-x scroll-with-animation :scroll-animation-duration="300">
			<view class="tabs-item" :class="{'tabs-flex':!isScroll, 'tabs-scroll':isScroll}" :style="{color: textColor}">
				<!-- tab -->
				<view class="tab-item" :style="{width: tabWidthVal, height: tabHeightVal, 'line-height':tabHeightVal}" v-for="(tab, i) in tabs" :class="{'active': value===i}" :key="i" @tap.stop="tabClick(i)">
					{{tab.title}}
				</view>
				<!-- 下划线 -->
				<view v-if="lines" class="tabs-line" :style="{left:lineLeft, background: textColor}"></view>
			</view>
		</scroll-view>
	</view>
</template>
