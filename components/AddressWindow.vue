  ...
  <view>
    <view class="address-window" :class="value === true ? 'on' : ''">
      ...
      <view class="list" v-if="addressList.length">
        <view class="item acea-row row-between-wrapper" :class="item.id === checked ? 'font-color-red' : ''" v-for="(item, addressIndex) in addressList" @click="tapAddress(addressIndex)" :key="addressIndex">
