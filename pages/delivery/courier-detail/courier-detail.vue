<template>
	<view class="courier-detail-content">
		...
		<view class="courier-detail-title-box">
			<view class="courier-detail-title">
				快递详情
			</view>
			<view class="courier-detail-subtitle">
				Courier Detail
			</view>
		</view>
				...
				<view class="uni-flex  uni-row form-box">
					...
					<view class="uni-flex uni-row form-item" style="display: flex;" v-if="deliveryRecord.state != 'SIGNED' && deliveryRecord.state != 'REFUSE'">
						...
						<view class="flex-item form-item-value">
							{{deliveryRecord.deliveryMethodName}}
						</view>
					</view>
					...
					<view @click.stop="phoneCall" class="uni-flex uni-row form-item" style="display: flex;" v-if="deliveryRecord.state === 'DELIVERING' || deliveryRecord.state === 'EXCEPTION'">
						<view class="flex-item form-item-label">
							派送信息
						</view>
						<view class="flex-item form-item-value">
							{{deliveryRecord.deliveryman + ' ' + deliveryRecord.deliverymanMobileNumber}}
						</view>
					</view>
					<view class="form-item-border" v-if="deliveryRecord.state === 'DELIVERING' || deliveryRecord.state === 'EXCEPTION'"></view>
					...
					<view class="uni-flex uni-row form-item" style="display: flex;" v-if="deliveryRecord.state === 'SIGNED' || deliveryRecord.state === 'REFUSE'">
						<view class="flex-item form-item-label">
							{{deliveryRecord.state === 'SIGNED'?'签收时间':'拒收时间'}}
						</view>
						<view class="flex-item form-item-value">
							{{deliveryRecord.signedTime}}
						</view>
					</view>
					...
					<view class="uni-flex uni-row form-item" style="display: flex;" v-if="deliveryRecord.state != 'SIGNED' && deliveryRecord.state != 'REFUSE'">
						...
						<view class="flex-item" style="color: #FF9F1E;margin-left: 24rpx; height: 40rpx;line-height: 40rpx;">
							请保持手机通畅
						</view>
					</view>
		...
		<view class="content-bg">
			<view :style="{marginTop: buttonTopMargin,marginBottom: buttonBottomMargin,marginLeft: '24rpx',marginRight: '24rpx'}"
			 class="uni-flex uni-row content-btn-box" v-if="deliveryRecord.state != 'SIGNED' && deliveryRecord.state != 'REFUSE'">
				<view class="flex-item">
					<button class="rest-date-btn" @click="showRestDate=true">更改派送时间</button>
				</view>
				<view class="flex-item" style="margin-left: 24rpx;">
					<button class="refuse-btn" @click="showRefuse=true">拒收</button>
				</view>
			</view>
		...
		<van-popup :show="showRestDate" bind:close="onClose">
			<view class="show-rest-date-box">
				...
				<view class="uni-flex uni-row show-rest-date-btn-box">
					<view class="flex-item">
						<button @click="showRestDate=false" class="show-rest-date-btn-l">取消</button>
					</view>
					<view class="flex-item">
						<button @click="showRestDate=false" class="show-rest-date-btn-r">好的</button>
					</view>
				</view>
			</view>
		...
		<van-popup :show="showRefuse" bind:close="onClose">
			<view class="show-refuse-box">
				...
				<view class="uni-flex uni-row show-refuse-btn-box">
					<view class="flex-item">
						<button @click="showRefuse=false" class="show-refuse-btn-l">取消</button>
					</view>
					<view class="flex-item">
						<button @click="confirmRefuse" class="show-refuse-btn-r">确认拒收</button>
					</view>
				</view>
			</view>
