			...
			<form>
				<view class="uni-flex  uni-row form-item">
					...
					<view class="flex-item">
						<input v-model="userAddress.realName" class="uni-input form-item-input" placeholder-style="color:#999999" name="input" placeholder="请输入" />
					</view>
				</view>
				...
				<view class="uni-flex  uni-row form-item">
					...
					<view class="flex-item">
						<input v-model="userAddress.houseNumber" class="uni-input form-item-input" placeholder-style="color:#999999" name="input" placeholder="如: A2" />
					</view>
				</view>
				...
				<view v-if="showWeekdays && showDeliveryTime" class="uni-flex  uni-row form-item-no-delivery-time">
					<view class="uni-flex">
						<view class="times-box">
							<view class="times-label">工作日</view>
							<view class="time-item-box">
								<view class="time-item" v-bind:key="index" v-for="(item, index) in noDeliveryTime.weekdays">{{ item.times }}</view>
							</view>
						</view>
						<view class="times-box" v-if="showHoliday && showDeliveryTime">
							<view class="times-label">节假日</view>
							<view class="time-item-box">
								<view class="time-item" v-bind:key="index" v-for="(item, index) in noDeliveryTime.holiday">{{ item.times }}</view>
							</view>
						</view>
					</view>
...
export default {
	...
	methods: {
		...
		showNoDeliveryTime(noDeliveryTime) {
			...
			if (noDeliveryTime) {
				if (
					(noDeliveryTime.weekdays && noDeliveryTime.weekdays.length && noDeliveryTime.weekdays.length > 0) ||
		...
		async submit() {
			...
			try {
				await this.$validator({
					...
					phone: [required(required.message('手机号')), chs_phone(chs_phone.message())],
					addressText: [required('楼栋号')],
